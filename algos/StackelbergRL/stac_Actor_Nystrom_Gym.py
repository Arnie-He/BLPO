...
from algos.core.Wrappers import GymnaxToGymWrapper
...
def run_rollout(env, length, actor_state, rng_key):
    ...
    jax.debug.print("starting rollout, length = {x}", x=length)
    ...
    for steps in range(length):
        ...
        debug.print("observation: {x}", x=next_observation)
        debug.print("action: {x}", x=action)
        debug.print("Done: {x}", x=done)
    
        # Print rewards
        # debug.print("Reward: {x}", x=reward)
    ...
    # Create a Transition object with array attributes
    ...
    return transitions, next_observation
...
def update_leaderactor(actor_state, critic_state, transitions, advantages, targets, last_observation, hyperparams, vanilla=False):
    ...
    def advantage_loss(params, transitions, advantages):
        ...
        log_probs = action_dists.log_prob(transitions.action + 1e-8)
...
def calc_episode_rewards(transitions):
    ...
    debug.print("Rewards: {x}", x=rewards)
...
def run_update(env, env_params, actor_state, critic_state, rng_key, hyperparams, vanilla=False):
    ...
    transitions, last_observation = run_rollout(env, hyperparams.rollout_len, actor_state, rollout_key)
    ...
    average_reward = jnp.sum(total_rewards * transitions.done) / jnp.sum(transitions.done + 1e-8)
...
def train(env_key, seed, logger, hyperparams, verbose=True, vanilla=False):
    ...
    print(hyperparams)
    rng_key, actor_key, critic_key = jax.random.split(jax.random.PRNGKey(seed), 3)

    envi = GYM_ENV_NAMES[env_key]
    # Create Gymnax environment and wrap it
    gymnax_env, env_params = gymnax.make(envi)  # Use env_key to select the environment
    env = GymnaxToGymWrapper(gymnax_env, params=env_params, seed=seed)

    # Initialize environment state
    observation_space = env.observation_space
    empty_observation = jnp.empty(observation_space.shape)
    # print(empty_observation)

    # Initialize actor model
    actor_model_params = models.params.init_gym(env, config["actor_params"])
    actor = config["actor_model"](*actor_model_params)
    actor_params = actor.init(actor_key, empty_observation)

    # Initialize critic model
    critic_model_params = models.params.init_gym(env, config["critic_params"])
