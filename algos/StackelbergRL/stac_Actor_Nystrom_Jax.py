...
def run_rollout(env, env_params, length, actor_state, rng_key):
    ...
    def step(rollout_state, x):
        ...
        # jax.debug.print("observation: {x}", x = observation)
...
def train(env_key, seed, logger, hyperparams, verbose=True, vanilla=False):
    ...
    elif (env_key in JUMANJI_ENV_NAMES):
        env = jumanji.make(JUMANJI_ENV_NAMES[env_key])
        empty_observation = jnp.empty(env.observation_spec)
        actor_model_params = models.params.init_jumanji(env,)
