...
def make_train(config):
    ...
    group_name = f'{config["Group"]}_{config["ENV_NAME"]}_NysCorrect'
        ...
        def _update_step(runner_state, unused):
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    total_gradient, grad_theta_J_norm, final_product_norm, original_ratio = hypergrad()
                    actor_state = actor_state.apply_gradients(grads=total_gradient)

                    # metrics = {
                    #     "actor_loss": actor_loss,
                    #     "critic_loss": critic_loss,
                    #     "first-order gradient norm": grad_theta_J_norm,
                    #     "final_product_norms": final_product_norm,
                    #     "final_product to first_order": original_ratio,
                    # }
                ...
                # mean_losses = jax.tree_map(lambda arr: jnp.mean(arr), loss_info)
            ...
            def callback(info):
                metric, loss_info = info

                # 1) get returns & steps
                returns   = metric["returned_episode_returns"][metric["returned_episode"]]
                timesteps = metric["timestep"][metric["returned_episode"]] * config["NUM_ENVS"]

                # T = len(timesteps)
                # # 2) flatten each [E, M] leaf → [E*M]
                # flat_info = jax.tree_map(lambda x: x.reshape(-1), loss_info)
                # # 3) pad (with zeros) or truncate to length T
                # def pad_to_T(arr):
                #     K = arr.shape[0]
                #     if T > K:
                #         pad = jnp.zeros((T - K,), dtype=arr.dtype)
                #         return jnp.concatenate([arr, pad], axis=0)
                #     else:
                #         return arr[:T]
                # padded = jax.tree_map(pad_to_T, flat_info)

                # # 4) log at each timestep
                # names = (
                #     "actor_loss",
                #     "critic_loss",
                #     "first-order gradient norm",
                #     "final_product_norms",
                #     "final_product to first_order",
                # )
                for t, step in enumerate(timesteps):
                    log_dict = {"Reward": float(returns[t])}
                    # for name, arr in zip(names, padded):
                    #     log_dict[name] = float(arr[t])
