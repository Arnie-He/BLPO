import wandb
import jax
from archive import dis_nystrom_ac
...
def main():
    ...
    train_jit = jax.jit(dis_nystrom_ac.make_train(config))
...
sweep_config = {
    ...
    "parameters": {
        ...
        "NUM_ENVS": {"value": 1},  # Fixed value
        "NUM_STEPS": {"value": 128},  # Fixed value
        "TOTAL_TIMESTEPS": {"value": 5e5},  # Fixed value
        "UPDATE_EPOCHS": {"value": 1},  # Fixed value
        "NUM_MINIBATCHES": {"value": 1},  # Fixed value
        ...
        "actor-LR": {"values": [2.5e-4, 3e-4]},  # Fixed value
        "critic-LR": {"values": [1e-3, 1.2e-3]},  # Fixed value
...
sweep_id = wandb.sweep(sweep=sweep_config, project="HyperGrad_AC")
