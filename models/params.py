...
def init_gymnax(env, env_params, model_params):
...
def init_jumanji(env, model_params):
    """Initializes dynamic model parameters given an environment."""
    for p in range(len(model_params)):
        if model_params[p] == DynParam.ActionCount:
            model_params[p] = env.action_spec.num_values
    return model_params

def init_gym(env, model_params):
    """Initializes dynamic model parameters given an environment."""
    for p in range(len(model_params)):
        if model_params[p] == DynParam.ActionCount:
            model_params[p] = env.action_space.n
    return model_params