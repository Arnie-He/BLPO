...
def make_train(config):
    ...
    group_name = f'{config["Group"]}_{config["ENV_NAME"]}_Nystrom_newgrad_diag'
                ...
                def _update_minbatch(train_state, batch_info):
                    ...
                    def leader_f2_loss(actor_params, critic_params, transitions, targets):
                        ...
                        for t in range(len(advantages)):
                            # Grab the tail of prob_ratios and advantages from t onward
                            prob_ratios_tail = prob_ratios[t:]
                            advantages_tail = advantages[t:]
                            # Compute the unclipped losses for this tail
                            unclipped_losses = prob_ratios_tail * advantages_tail
                            # Compute the clipped losses for this tail
                            clipped_ratios = jnp.clip(prob_ratios_tail, 1 - config["CLIP_F"], 1 + config["CLIP_F"])
                            clipped_losses = clipped_ratios * advantages_tail
                            # For the loss vector starting at time t, take the elementwise minimum
                            loss = jnp.minimum(unclipped_losses, clipped_losses)
                            loss_t = -jnp.mean(loss)
                            ppo_losses.append(loss_t)
                        ppo_losses = jnp.array(ppo_losses)
                    ...
                    def hypergrad():
                        ...
                        def nystrom_hvp(rank, rho, rng):
                            # (a) v = ∂_φ J_PPO
                            in_out_g = jax.grad(ppo_loss, argnums=1)(
                                actor_state.params, critic_state.params, traj_batch
                            )
                            v_flat, _ = jax.flatten_util.ravel_pytree(in_out_g)

                            # (b) importance‑sample indices with GN diag
                            indices = sample_indices_gn(critic_state.params, traj_batch, rng, rank)

                            # (c) true Hessian rows at those indices
                            def select_grad_row(p, idxs):
                                g = jax.grad(lambda q: critic_target_loss(q, traj_batch, targets))(p)
                                g_flat, _ = jax.flatten_util.ravel_pytree(g)
                                return g_flat[idxs]

                            # NB: argnums=0 so we only differentiate wrt 'params'
                            H_rows = jax.jacrev(select_grad_row, argnums=0)(critic_state.params, indices)
                            C, _   = jax.flatten_util.ravel_pytree(H_rows)
                            C      = C.reshape(rank, -1)             # shape (r, P)
                            M      = C.take(indices, axis=1)         # shape (r, r)

                            # (d) Nystrom inverse solve
                            A = M + (1/rho) * (C @ C.T) + jnp.eye(rank)
                            y = jax.scipy.linalg.solve(A, C @ v_flat)          # small r×r solve
                            x = (1/rho) * v_flat - (1/rho**2) * (C.T @ y)      # ≈ H⁻¹ v
                            return x


    # … JVP, clipping, actor update exactly as you had …


                        # call it with a fresh key
                        inverse_hvp_flat = nystrom_hvp(
                            config["nystrom_rank"],
                            config["nystrom_rho"],
                            rng
                        )
