...
def make_train(config):
    ...
    group_name = f'{config["Group"]}_{config["ENV_NAME"]}_CG_GN'
        ...
        def _update_step(runner_state, unused):
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def gnvp(v):
                        # Flatten the critic parameters and obtain an unflattening function.
                        critic_params_flat, unravel_fn = jax.flatten_util.ravel_pytree(critic_state.params)
                        
                        # Define a function that, given flat parameters, computes the critic predictions
                        # for all observations in the trajectory.
                        def f(p):
                            params = unravel_fn(p)
                            # Use vmap to apply the critic network to each observation.
                            preds = jax.vmap(lambda obs: critic_network.apply(params, obs))(traj_batch.obs)
                            return preds

                        # Compute the Jacobian-vector product: u = J_f(p) * v
                        u = jax.jvp(f, (critic_params_flat,), (v,))[1]
                        
                        # Compute the vector-Jacobian product (the adjoint): hv = J_f(p)^T * u
                        _, vjp_fun = jax.vjp(f, critic_params_flat)
                        hv = vjp_fun(u)[0]
                        
                        # Add the regularization (damping) term.
                        return hv + config["lambda_reg"] * v

                    
                    grad_w_J_flat, unflatten_fn = jax.flatten_util.ravel_pytree(grad_w_J)
                    def cg_solve(v):
                        return jax.scipy.sparse.linalg.cg(gnvp, v, maxiter=20, tol=1e-10)[0]
