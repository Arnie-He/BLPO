...
def make_train(config):
    ...
    group_name = f'{config["Group"]}_{config["ENV_NAME"]}_Nystrom_gn'
                ...
                def _update_minbatch(train_state, batch_info):
                    ...
                    def hypergrad():
                        ...
                        def nystrom_hvp(rank, rho):
                            ...
                            def select_prediction_row(in_params, indices):
                                # Compute the critic's predictions for all observations.
                                preds = jax.vmap(lambda obs: critic_network.apply(in_params, obs))(traj_batch.obs)
                                # Flatten the predictions.
                                flat_preds, _ = jax.flatten_util.ravel_pytree(preds)
                                # Return only the selected indices.
                                return flat_preds[indices]

                            #actually the gn
                            hessian_rows = jax.jacrev(select_prediction_row)(critic_state.params, indices)
                        ...
                        hypergradient = jax.tree_util.tree_map(lambda x, y: x - 2*y, grad_theta_J, clipped_final_product)
