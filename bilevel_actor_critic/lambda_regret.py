...
def run_update(env, env_params, actor_state, critic_state, rng_key, hyperparams, vanilla=False):
    ...
    # advantage loss
    # adv loss differentiable to critic_params
    def f(actor_params, critic_params):
    ...
    # leader-f2-loss
    def g1(actor_params, critic_params):
        advantages, _ = calc_values(critic_state, critic_params, transitions, last_observation, hyperparams.discount_rate)
        action_dists = jax.vmap(actor_state.apply_fn, in_axes=(None, 0))(actor_params, transitions.observation)
        log_probs = action_dists.log_prob(transitions.action)
        ...
        return result
    # target-loss
    def g2(critic_params, targets):
        """Calculates the mean squared error on a batch of transitions."""
        values = jax.vmap(critic_state.apply_fn, in_axes=(None, 0))(critic_params, transitions.observation)
        errors = jnp.square(targets - values)
        return jnp.mean(errors)
    # define regret target
    def L1(actor_params, critic_params, critic_params_star, lam):
        return f(actor_params, critic_params) + lam * (g1(actor_params, critic_params) - g1(actor_params, critic_params_star))
    def L2(actor_params, critic_params, critic_params_star, targets, lam):
        return f(actor_params, critic_params) + lam * (g2(critic_params, targets) - g2(critic_params_star, targets))
    
    _, targets = calc_values(critic_state, critic_state.params, transitions, last_observation, hyperparams.discount_rate)

    star_critic_state = critic_state
    for c in range(hyperparams.nested_updates):
        grads = jax.grad(g2)(star_critic_state.params, targets)
        star_critic_state = star_critic_state.apply_gradients(grads=grads)
    
    lam = 100000
    actor_grad = jax.grad(L1)(actor_state.params, critic_state.params, star_critic_state.params, lam)
    critic_grad = jax.grad(L2, argnums=1)(actor_state.params, critic_state.params, star_critic_state.params, targets, lam)

    actor_state = actor_state.apply_gradients(grads=actor_grad)
    critic_state = critic_state.apply_gradients(grads=critic_grad)
    
    critic_state = star_critic_state

    actor_loss = f(actor_state.params, critic_state.params)
    critic_loss = g2(critic_state.params, targets)
...
def train(env_key, seed, logger, verbose):
    ...
    for b in range(num_batches):
        ...
        if verbose:
            print(f"[Update {(b + 1) * hyperparams.batch_count}]: Average reward {batch_metrics[0][-1]}, actor_loss: {batch_metrics[2][-1]}, critic_loss: {batch_metrics[3][-1]}")