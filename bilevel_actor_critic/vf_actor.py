...
@jax.jit
def calc_values(critic_params, critic_state, transitions, last_observation, discount_rate):
...
# follower-objective
def g(params, transitions, targets, critic_state):
...
def f(actor_params, actor_state, critic_params, critic_state, transitions, last_observation, hyperparams):
    advantages, targets = calc_values(critic_params, critic_state, transitions, last_observation, hyperparams.discount_rate)
...
def surrogate_loss(actor_params, critic_params, actor_state, critic_state, transitions, advantages, targets):
        action_dists = jax.vmap(actor_state.apply_fn, in_axes=(None, 0))(actor_params, transitions.observation)
        log_probs = action_dists.log_prob(transitions.action)
        values = jax.vmap(critic_state.apply_fn, in_axes=(None, 0))(critic_params, transitions.observation)
        return 2 * jnp.mean(log_probs * advantages) * (targets[0] - values[0])

def L(actor_params, actor_state, critic_params, critic_state, transitions, last_observation, hyperparams, lam, targets):
    return f(actor_params, actor_state, critic_params, critic_state, transitions, last_observation, hyperparams) + lam * (g(critic_params, transitions, targets, critic_state) - g(critic_params_star, transitions, targets, critic_state))

def update(actor_state, critic_state, transitions, last_observation, hyperparams, vanilla=False):
