...
def update_critic(critic_state, transitions, last_observation, gamma):
    ...
    advantages, targets = calc_values(critic_state, transitions, last_observation, gamma)
    target_grad = jax.value_and_grad(target_loss)
    critic_loss, grads = target_grad(critic_state.params, transitions, targets, critic_state)
    critic_state = critic_state.apply_gradients(grads=grads)
    return (critic_state, advantages, critic_loss)

def learn_critic_star(critic_state, transitions, last_observation, gamma, num_iterations=20):
    for _ in range(num_iterations):
        critic_state, advantages, critic_loss = update_critic(critic_state=critic_state, transitions=transitions, last_observation=last_observation, gamma=gamma)
    return (critic_state, advantages, critic_loss)

def bilevel_actor(actor_params, actor_state, critic_state, transitions, last_observation, gamma):
    ...
    critic_state, advantages, critic_loss = learn_critic_star(critic_state=critic_state, transitions=transitions, last_observation=last_observation, gamma=gamma)
    return advantage_loss(actor_params, transitions, advantages)
    
def update_leaderactor(actor_state, critic_state, transitions, last_observation, gamma, vanilla=False):
    
    value_grad = jax.value_and_grad(bilevel_actor)
    actor_loss, actor_grad = value_grad(actor_state.params, actor_state, critic_state, transitions, last_observation, gamma)
    actor_state.apply_gradients(grads=actor_grad)

    critic_state, advantages, critic_loss = learn_critic_star(critic_state, transitions, last_observation, gamma)
...
def run_update(env, env_params, actor_state, critic_state, rng_key, hyperparams, vanilla=False):
    ...
    actor_state, actor_loss, critic_state, critic_loss = update_leaderactor(actor_state, critic_state, transitions, last_observation, vanilla)
    ...
    return (actor_state, critic_state, (average_reward, 0, actor_loss, critic_loss), rng_key)
...
def train(env_key, seed, logger, verbose = False, metrics=None, vanilla=False, save_charts=False, description=None):
    ...
    if(save_charts):
        ...
        folder_path = f"charts/envelop_Actor/{description}"
    ...
    for b in range(num_batches):
        ...
        if verbose:
            print(f"[Update {(b + 1) * hyperparams.batch_count}]: Average reward {batch_metrics[0][-1]}, actor loss {jnp.mean(batch_metrics[2])}, critic loss{jnp.mean(batch_metrics[3])}")