...
def update_critic(critic_state, transitions, last_observation, gamma):
    ...
    return (critic_state, advantages)

def learn_critic_star(env, env_params, rollout_key, actor_state, critic_state, hyperparams, num_iterations=20):
    transitions, last_observation = run_rollout(env, env_params, hyperparams.rollout_len, actor_state, rollout_key)
    for _ in range(num_iterations):
        critic_state, advantages = update_critic(critic_state=critic_state, transitions=transitions, last_observation=last_observation, gamma=hyperparams.discount_rate)
    return critic_state, advantages, transitions

def bilevel_actor(actor_params, actor_state, critic_state, env, env_params, rollout_key, hyperparams):
    ...
    critic_state, advantages, transitions = learn_critic_star(env=env, env_params=env_params, rollout_key=rollout_key, actor_state=actor_state, critic_state=critic_state, hyperparams=hyperparams)
    return advantage_loss(actor_params, transitions, advantages)
    
def update_leaderactor(actor_state, critic_state, env, env_params, rollout_key, hyperparams, vanilla=False):
    ...
    actor_loss, actor_grad = value_grad(actor_state.params, actor_state, critic_state, env, env_params, rollout_key, hyperparams)
...
def run_update(env, env_params, actor_state, critic_state, rng_key, hyperparams, vanilla=False):
    ...
    actor_state, actor_loss = update_leaderactor(actor_state, critic_state, env, env_params, rollout_key, hyperparams, vanilla)
    critic_state, advantages, transitions = learn_critic_star(env, env_params, rollout_key, actor_state, critic_state, hyperparams)
    ...
    return (actor_state, critic_state, (average_reward, 0, 0, 0), rng_key)
