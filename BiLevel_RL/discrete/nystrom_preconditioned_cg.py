...
def make_train(config):
    ...
    wandb.init(project="HyperGradient-RL", name=run_name(config), config = config)
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def nystrom_hvp(v):
                        ...
                        indices = jax.random.permutation(jax.random.PRNGKey(0), param_size)[:config["nystrom_rank"]]
                        ...
                        C = jnp.reshape(hessian_rows_flat, (config["nystrom_rank"], -1))
                        M = C.take(indices, axis=1)
                        v_flat, _ = jax.flatten_util.ravel_pytree(v)
                        x = (1 / (config["nystrom_rho"] )) * v_flat - (1 / ((config["nystrom_rho"]) ** 2)) * C.T @ jax.scipy.linalg.solve(M + (1 / config["nystrom_rho"]) * C @ C.T +  jnp.eye(M.shape[0]), C @ v_flat)
                        return x
                    ...
                    grad_w_J = jax.grad(ppo_loss, 1)(actor_state.params, critic_p, traj_batch)
                    ...
                    def hvp(v):
                        ...
                        lambda_reg = 1.0 / config["nystrom_rho"]
                    ...
                    def cg_solve(v):
                        return jax.scipy.sparse.linalg.cg(hvp, v, M=nystrom_hvp, maxiter=10, tol=1e-10)[0]
